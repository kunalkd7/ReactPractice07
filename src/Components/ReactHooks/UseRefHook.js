...
export function UseRefHook() {
  //useRef is similar to usestate but it will not cause rerender of page
  ...
  //basically it is a ref of input field 
  //we can create any dom element ref with useRef h
  const inputRef = useRef(null);

  const handleSubmit = () => {
    console.log(inputRef.current.value);
    //we can also assign new val inputRef.current.value = "anything"
  };
  ...
  return (
    <>
      <button onClick={() => setRender((prevr) => prevr + 1)}>click me</button>
      {render}
      <button onClick={() => setRenderOne((prevr) => prevr + 1)}>
        click me
      </button>
      {renderOne} <br />
